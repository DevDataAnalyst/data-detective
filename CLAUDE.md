...
| data      | `npm run generate:data` (rewrites every mission CSV in `public/data/`)  |
...
- Code in questions is never trusted either. A multiple choice question can show `code` (SQL or
  Python, lines of at most 48 characters, `____` for a blank) and SQL `tables` (each caption is a
  table name, `null` is NULL). Its `check` says how the answer is proven: `sql_value` (the query
  returns the correct option's number), `sql_rows` (that many rows), `sql_blank` (only the
  correct option, put in the blank, returns the same rows as a `reference` query), and
  `python_output` or `python_blank` for pandas. `src/test/sqlQuestions.test.ts` runs every SQL
  check in Node's built-in SQLite (`node:sqlite`; column types follow pandas, so whole numbers
  divide as whole numbers), and `src/test/pythonQuestions.pyodide.test.ts` runs every Python
  one in Pyodide (`npm run test:python`). Both walk all content: lessons, checkpoints, mission
  question tasks, daily challenges and previews.

## Challenge question types

- Six types frame questions as analyst work: `inbox_triage` (a vague ask as a chat or email
  ...
  cards, some of them distractors), `ab_verdict` (an A/B test's counts; the learner calls
  ship, kill or wait and sees the `consequences` of the call they picked) and `order_steps` (3–7
  steps of a method, or lines of code with `language`, tapped into order).
- `order_steps` is a word bank: tapping a step adds it to the end of the order, tapping a placed
  step takes it back, number keys place the nth step left and Backspace takes back the last. The
  offer order is a shuffle seeded by the question id (`stepOfferOrder` in
  `src/game/orderSteps.ts`), never already right. SQL lines with `tables` and a `reference` must
  run, in order, to the reference's rows. Steps must have one right order: make each depend on
  the one before.
...
- Missions are `src/content/mission1.ts` to `mission4.ts` (validated by `validateMission`).
  Their datasets are generated by `npm run generate:data`; the planted patterns are documented in
  `scripts/README.md`. Every number content quotes from a dataset is checked against the CSV in
  tests (`src/test/deliveriesReference.ts`, `churnReference.ts`, `checkoutReference.ts` and
  `interviewReference.ts`).
- A mission can use several files: `dataset` (the main one, which the checks work from) and
  `extraData`. A file with a `table` is also loaded into a read-only in-memory SQLite database
  (`load_tables` in `runner.py`, `PRAGMA query_only`), and code tasks with `language: 'sql'` are
  one query over those tables. `codeToRun` (`src/mission/sqlTasks.ts`) turns a SQL task into
  `name = sql("...")`, so the query runs, is checked and is replayed exactly like Python, and
  its result is saved as the task's one `creates` variable for later tasks. SQL errors come back
  as "SQL error" with no Python line numbers. The editor switches to SQLite highlighting.
...
- Hidden checks live in one Python module per mission (`checks.py`, `checks_churn.py`,
  `checks_checkout.py`, `checks_interview.py`), listed in `src/mission/python/checkModules.ts`; the
  worker loads the one for the open mission. At load, `compute_reference` works out every reference
  answer from the CSV (never hard-coded), and `reference_summary()` sends the dataset facts to the
  page. Each mission lists its `facts`, the only placeholders its summary lines may use, and a
  Python test checks the two agree. After each run with a `taskId`, the worker calls `check_task`,
  which returns `{passed, message}` and never raises. Messages point at what to fix without giving
  the answer. `npm run test:python` runs every check against correct and deliberately wrong
  solutions in Pyodide under Node (needs the network the first time).
...
- `journey.spec.ts` is the whole path: onboarding, lesson 1, testing out, and the first three
  ...
  Pyodide and passes its first code task (Unit 4's is SQL): the fake worker in unit tests cannot
  catch a dataset that fails to load. `python-load.spec.ts` measures how long Python takes to load,
...
- Unit 4, "Crack the Interview" (`unit4.ts`), SQL and pandas interview questions and a
  six-step routine for any data problem (Clarify, Inspect, Plan, Build, Check, Explain),
  mission "The Final Round" (`mission4.ts`; `customers.csv`, `orders.csv` and
  `order_items.csv`). It opens with an email `hook` from Kavya, the analytics lead at Nashta Now,
  a breakfast delivery app, inviting the learner to a technical round. The twist: the finance
  dashboard joins orders to order_items before adding up order value, so Pune's big combo orders
  count about 3.7 times each; counted once, Mumbai brought in the most. Error hints name the
  open mission's own files, tables and variables (`ErrorHintContext`).
